...
#define TEST_GET_OR_THROW_VALID(TYPE, TYPENAME_STRING, KEY, EXPECTED)  \
 ...
 string_vec_var: ["abc", "def"]
    ...
    TEST_GET_OR_THROW_VALID(std::vector<float>, "std::vector<float>", "float_vec_var", (std::vector<float>{3.14f,2.71f}) )
    TEST_GET_OR_THROW_VALID(std::vector<double>, "std::vector<double>", "double_vec_var", (std::vector<double>{3.14,2.71}) )
    TEST_GET_OR_THROW_VALID(std::vector<std::string>, "std::vector<std::string>", "string_vec_var", (std::vector<std::string>{"abc", "def"}) )
...
#define TEST_GET_OPT_VALID(TYPE, TYPENAME_STRING, KEY, EXPECTED)           \
 ...
 string_vec_var: ["abc", "def"]
    ...
    TEST_GET_OPT_VALID(std::vector<std::string>, "std::vector<std::string>", "string_vec_var", (std::vector<std::string>{"abc", "def"}) )
...
#define TEST_SET_OR_THROW(TYPE, TYPENAME_STRING, KEY, EXPECTED)  \
    ...
    using metkit::mars2grib::utils::dict_traits::get_or_throw;    \
        TYPE expected_result = (EXPECTED);                        \
    EXPECT_NO_THROW({                                             \
        set_or_throw<TYPE>(cfg, (KEY), expected_result);          \
    });                                                           \
    EXPECT_NO_THROW({                                             \
        TYPE actual_result = get_or_throw<TYPE>(cfg, (KEY) );     \
        EXPECT_EQUAL(actual_result, expected_result);             \
    ...
    TEST_SET_OR_THROW(std::vector<std::string>, "std::vector<std::string>", "string_vec_var", (std::vector<std::string>{"abc", "def"}) );
...
#define TEST_SET_OR_IGNORE(TYPE, TYPENAME_STRING, KEY, EXPECTED)  \
    ...
    using metkit::mars2grib::utils::dict_traits::get_or_throw;    \
        TYPE expected_result = (EXPECTED);                        \
    EXPECT_NO_THROW({                                             \
        set_or_ignore<TYPE>(cfg, (KEY), expected_result);         \
    });                                                           \
    EXPECT_NO_THROW({                                             \
        TYPE actual_result = get_or_throw<TYPE>(cfg, (KEY) );     \
        EXPECT_EQUAL(actual_result, expected_result);             \
    ...
    TEST_SET_OR_IGNORE(std::vector<std::string>, "std::vector<std::string>", "string_vec_var", (std::vector<std::string>{"abc", "def"}) );
...
#define TEST_TYPED_HAS_MISSING(TYPE, TYPENAME_STRING, KEY){   \
    ...
    SECTION("has<" TYPENAME_STRING ">") {                     \
        bool actual_has;                                      \
        EXPECT_NO_THROW({                                     \
            actual_has = has<TYPE>(cfg, "missing_key" );      \
        });                                                   \
        EXPECT(!actual_has);                                  \
    }                                                         \
    }
...
#define TEST_CHECK_VALID(TYPE, TYPENAME_STRING, KEY)      \
    ...
    SECTION("check<" TYPENAME_STRING ">") {               \
        EXPECT_NO_THROW({                                 \
            bool actual_has = check<TYPE>(cfg, (KEY),     \
...
using metkit::mars2grib::utils::dict_traits::get_or_throw;
 ...
 string_vec_var: ["abc", "def"]
    ...
    SECTION("set sub-configuration n1") {
        EXPECT_NO_THROW({
            set_or_throw<eckit::LocalConfiguration>(cfg, "n1", n1);
        });
        EXPECT_NO_THROW({
            eckit::LocalConfiguration actual_n1 = get_or_throw<eckit::LocalConfiguration>(cfg, "n1");
        });
    }

    SECTION("set sub-configuration n2") {
        EXPECT_NO_THROW({
            set_or_throw<eckit::LocalConfiguration>(cfg, "n2", n2);
        });
        EXPECT_NO_THROW({
            eckit::LocalConfiguration actual_n2 = get_or_throw<eckit::LocalConfiguration>(cfg, "n2");
        });
    }
    SECTION("set sub-configurations") {
        EXPECT_NO_THROW({
            set_or_throw<std::vector<eckit::LocalConfiguration>>(cfg, "n3", n3);
        });
        EXPECT_NO_THROW({
            std::vector<eckit::LocalConfiguration> actual_n3 = get_or_throw<std::vector<eckit::LocalConfiguration>>(cfg, "n3");
        });
    }
