...
struct MarsLanguageValidationError : public eckit::Exception {
    MarsLanguageValidationError(const std::string& msg,
                                const eckit::CodeLocation& loc = Here()) :
        eckit::Exception(msg, loc) {}
};

void validateMarsKeyValue(const eckit::Value& language,
                          ...
                          const std::string& value)
{
    ...
    if (!language.contains(key)) {
        throw MarsLanguageValidationError(
            "Unknown MARS key: '" + key + "'"
        );
    }
    ...
    // --------------------------------------------------
    throw MarsLanguageValidationError(
        "Invalid value '" + value + "' for MARS key '" + key + "'"
    );
}
...
int main()
{
    try {
        const eckit::Value languageYaml =
            eckit::YAMLParser::decodeFile(
                eckit::PathName("/ec/res4/hpcperm/mavm/ba/metkit-bundle/language.yaml")
            );
