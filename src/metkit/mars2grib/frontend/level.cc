...
void setSFC(const eckit::LocalConfiguration& mars, eckit::LocalConfiguration& sections) {
    ...
    if (matchAny(param, 228023)) {
        ...
        setTypeOfLevel(sections, "soilLayer");
                 ...
                 range(243, 245), 3020, 3062, 3067, 3099, range(140098, 140105), range(140112, 140129),
...
void setPL(const eckit::LocalConfiguration& mars, eckit::LocalConfiguration& sections) {
    const auto param = *get<std::int64_t>(mars, "param");
    const auto level = *get<std::int64_t>(mars, "level");

    if (matchAny(param, 1, 2, 10, 60, 75, 76, range(129, 135), 138, 152, 155, 157, 203, range(246, 248), 260290)) {
        if (level >= 100) {
