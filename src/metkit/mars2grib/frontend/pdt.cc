...
struct PDTCat {
    ...
    bool operator==(const PDTCat& other) const noexcept {
        return std::tie(timeExtent, timeFormat, spatialExtent,
                        processType, processSubType,
                        productCategory, productSubCategory)
             == std::tie(other.timeExtent, other.timeFormat, other.spatialExtent,
                         other.processType, other.processSubType,
                         other.productCategory, other.productSubCategory);
    }
};
...
PDTCat parsePDTCat(const eckit::LocalConfiguration& pdt) {
    return {
        parseTimeExtent(*get<std::string>(pdt, "timeExtent")),
