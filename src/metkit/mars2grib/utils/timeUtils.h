...
namespace metkit::mars2grib::utils::time {

// =============================================================
// Minimal enums CodeTAble 4.4
// =============================================================
...
inline Period decodePeriod_orThrow(std::string_view s)
{
    ...
    if (s == "da") return Period::Daily;
    if (s == "mo") return Period::Monthly;
    throw Mars2GribGenericException(
            "Invalid period token: " + std::string(s),
                Here()
            );
}
...
inline StatOp decodeOp_orThrow(std::string_view s)
{
    ...
    if (s == "av") return StatOp::Average;
    if (s == "mn") return StatOp::Minimum;
    if (s == "mx") return StatOp::Maximum;
    if (s == "sd") return StatOp::StdDev;
    throw Mars2GribGenericException(
            "Invalid operation token: " + std::string(s),
                Here()
            );
}
...
inline std::vector<StatTypeBlock>
parseStatType_or_throw(const std::string& stattype)
{
    using metkit::mars2grib::utils::exceptions::Mars2GribGenericException;
    ...
    while (pos < stattype.size()) {
        ...
        if (pos < stattype.size()) {
            if (stattype[pos] != '_') {
                throw Mars2GribGenericException(
                        "Invalid stattype separator (expected '_')",
                        Here()
                    );
            }
            ++pos;
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) {
        if (blocks[i].period == Period::Monthly) {
            if (moIndex != -1)
                throw Mars2GribGenericException(
                        "Invalid stattype: more than one 'mo'",
                        Here()
                    );
        ...
        if (blocks[i].period == Period::Daily) {
            if (daIndex != -1)
                throw Mars2GribGenericException(
                        "Invalid stattype: more than one 'da'",
                        Here()
                    );
    ...
    if (moIndex != -1 && daIndex != -1 && moIndex > daIndex) {
        throw Mars2GribGenericException(
                "Invalid stattype order: 'mo' must precede 'da'",
                Here()
            );
    }
...
{
    switch (op) {
        case StatOp::Average: return "Average";
        case StatOp::Minimum: return "Minimum";
        case StatOp::Maximum: return "Maximum";
        case StatOp::StdDev:  return "StandardDeviation";
    }
    return "Unknown";
}
...
inline void prettyPrint(const std::vector<StatTypeBlock>& blocks)
{
    ...
    for (std::size_t i = 0; i < blocks.size(); ++i) {
        std::cout << "  [" << i << "] "
                  << "Period = " << toString(blocks[i].period)
                  << ", Operation = " << toString(blocks[i].op)
                  << '\n';
    }
}
...
inline long toSeconds_or_throw(std::string_view step)
{
    ...
    if (pos == 0) {
        throw Mars2GribGenericException(
                "Invalid step format (no numeric part): " + std::string(step),
                Here()
            );
    }
    ...
    try {
        ...
        throw Mars2GribGenericException(
                "Invalid numeric value in step: " + std::string(step),
                Here()
            );
    }
    ...
    if (pos < step.size()) {
        if (pos + 1 != step.size()) {
            throw Mars2GribGenericException(
                "Invalid step format (trailing characters): " + std::string(step),
                Here()
);
        }
        unit = step[pos];
    ...
    switch (unit) {
        ...
        default:
            throw Mars2GribGenericException(
                std::string("Unknown step unit: '") + unit + "'",
                Here()
            );
    }
}
