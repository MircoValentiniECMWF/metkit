...
Section lookupCfgSection( const eckit::LocalConfiguration& cfg, long SecId ) {
    ...
    eckit::LocalConfiguration sectionCfg =
        cfg.getSubConfiguration( sectionName );
    ...
    sec.templateNumber_ =
        static_cast<uint16_t>(
            sectionCfg.getLong( "template-number" )
        );
    ...
    for ( auto& name : sectionCfg.keys() ) {
        ...
        if ( name == "template-number" ) {
            // std::cout << "   - Skipping template number" << std::endl;
        ...
        // Get the type
        eckit::LocalConfiguration conceptCfg =
            sectionCfg.getSubConfiguration( name );
        ...
        //std::cout << " - Processing concept name [Section:"<< std::to_string(SecId) <<"]: " <<
        // std::string(name) <<", key: " << key << ", type: " << type << std::endl;
        ...
        if ( key == "model" ){
            ...
            std::string typeOfStatisticalProcess =
            ...
            if ( typeOfStatisticalProcess == "average" ){
                ...
                throw Mars2GribGenericException(
                    "Unsupported type-of-statistical-processing "
                    + typeOfStatisticalProcess
                    + " for concept " + name,
                    Here( )
                );
            }
            ...
            // std::cout << "   - Inserting concept key " << "[Section:" << std::to_string(SecId) << "]: " << k << ", type: " << t << std::endl;
            sec.concepts_.insert( std::make_pair( key, type ) );
        }

    } // for concept name

//     for (const auto& [key, type] : sec.concepts_) {
//         std::cout << "   - Concept key " << "[Section:" << std::to_string(SecId) << "]: " << std::string(key) << ", type: " << std::string(type) << std::endl;
//     }
...
Section lookupExpectedSection( const eckit::LocalConfiguration& cfg, long SecId ) {
    ...
    if (concepts) {
        ...
        throw Mars2GribGenericException(
            "No concepts found for " + sectionName + " template number "
            + std::to_string(sec.templateNumber_),
            Here( )
        );
    }
...
EncoderCfg parseEncoderCfg( const eckit::LocalConfiguration& cfg ) {
    ...
    for ( size_t i = 0; i < NUM_SECTIONS; ++i ) {
        ...
        if ( encoderCfg.sec_[i].templateNumber_ != expectedCfg.sec_[i].templateNumber_ ) {
            throw Mars2GribGenericException(
                "Template number mismatch for section ",
                Here( )
            );
        }
        combinedCfg.sec_[i].templateNumber_ = encoderCfg.sec_[i].templateNumber_;

        //std::cout << "Combining section " << i << " :: " << expectedCfg.sec_[i].concepts_.size() << " expected concepts, "
        //          << encoderCfg.sec_[i].concepts_.size() << " provided concepts."
        //          <<    std::endl;

        // Combine concepts
        for ( const auto& [key, type] : expectedCfg.sec_[i].concepts_ ) {
            std::cout << "Combining concept " << key << " with type " << type << std::endl;
