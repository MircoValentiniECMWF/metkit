...
WaveSpectraInfo waveSpectraInfo_or_throw(
    const OptDict_t& opt,
    ...
    try {
        ...
        if ( hasNumberOfWaveDirections && !hasWaveDirections ) {
            throw Mars2GribDeductionException(
                "Not implemented! Getting wave directions from numberOfWaveDirections is still not supported",
                Here()
            );
        }
            ...
            for ( std::size_t i = 0; i < waveDirections.size(); ++i ) {
                out.scaledValuesDirections[i] = static_cast<long>(std::round( waveDirections[i] * std::pow(10.0, scaleFactorOfWaveDirections) ) );
            }
        } else if ( hasNumberOfWaveDirections && hasWaveDirections ) {
            throw Mars2GribDeductionException(
        ...
        if ( hasNumberOfWaveFrequencies && !hasWaveFrequencies ) {
            ...
            for ( std::size_t i = 0; i < waveFrequencies.size(); ++i ) {
                out.scaledValuesFrequencies[i] = static_cast<long>( std::round( waveFrequencies[i] * std::pow(10.0, scaleFactorOfWaveFrequencies) ) );
            }
        } else if ( hasNumberOfWaveFrequencies && hasWaveFrequencies ) {
            throw Mars2GribDeductionException(
        ...
        std::throw_with_nested(
            Mars2GribDeductionException(
                "Unable to get wave spectra information from Mars and Par dictionaries",
                Here()
            )
        );
