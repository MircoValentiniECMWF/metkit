...
inline StatisticalProcessing
getTimeDescriptorFromMars_orThrow(
    const MarsDict& mars,
    const ParDict&  par,
    ...
    try {
        ...
        if (numberOfTimeRangesVal < 1 || numberOfTimeRangesVal > 3) {
            throw Mars2GribDeductionException(
                    "Unexpected number of time loops",
                    Here()
                );
        }
        ...
        if ( timeSpanInSeconds % 3600 != 0 ) {
            throw Mars2GribDeductionException(
                    "`timespan` must be multiple of 3600 seconds",
                    Here()
                );
        }
        ...
        for (long i = 0; i < numberOfTimeRangesVal; ++i) {
            ...
            if (isInnerLoop) {
                ...
                out.typeOfStatisticalProcessing[i] =
                    static_cast<long>(blk.op);
                ...
                if (blk.period == Period::Daily) {
                    ...
                    out.lengthOfTimeRange[i] =
                        previousMonthLengthHours(endYear, endMonth);
                }
                else {
                ...
                if (i == numberOfTimeRangesVal - 2) {
                    ...
                    if (nextBlk.period == Period::Daily) {
                        ...
                        out.lengthOfTimeIncrement[i] =
                            previousMonthLengthHours(endYear, endMonth) * 3600;
                    }
                    else {
                        throw Mars2GribDeductionException(
                            "Unsupported next period",
                            Here()
                        );
                    }
                }
        ...
        std::throw_with_nested(
            Mars2GribDeductionException(
                "Unable to compute statistics descriptor from Mars dictionary",
                Here()
            )
        );
