...
inline const std::vector<SectionRecipe>*
recipesForSection(uint16_t sectionId)
{
    switch (sectionId) {
        case 0: return &Sec0Recipes;
        case 1: return &Sec1Recipes;
        case 2: return &Sec2Recipes;
        case 3: return &Sec3Recipes;
        case 4: return &Sec4Recipes;
        case 5: return &Sec5Recipes;
...
inline const SectionRecipe*
findRecipe(uint16_t sectionId, uint16_t templateNumber)
{
    const auto* recipes = recipesForSection(sectionId);
    if (!recipes) return nullptr;
