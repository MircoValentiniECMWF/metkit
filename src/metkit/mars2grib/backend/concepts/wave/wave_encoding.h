...
{
   bool condition1 = ( Section == SecProductDefinitionSection &&
                       Stage == StageAllocate &&
                       Variant == WaveType::Spectra );

   bool condition2 = ( Section == SecProductDefinitionSection &&
                       Stage == StagePreset &&
                       Variant == WaveType::Period );

    bool condition3 = ( Section == SecProductDefinitionSection &&
                       Stage == StageRuntime &&
                       Variant == WaveType::Spectra );
    ...
    if constexpr ( waveApplicable<Stage, Section, Variant>() ) {
        ...
        try {
            ...
            LOG_DEBUG_LIB(LibMetkit)
                << "[Concept Wave] Op called: "
                << "Stage="   << Stage
                << ", Section=" << Section
                << ", Variant=" << std::string(waveTypeName<Variant>())
                << std::endl;
            ...
            if constexpr ( Stage == StageAllocate ) {
                ...
                if constexpr ( Variant == WaveType::Spectra ) {
                    ...
                    set_or_throw<std::vector<long>>( out, "scaledValuesOfWaveDirections", spectraInfo.scaledValuesDirections );
                    ...
                    set_or_throw<std::vector<long>>( out, "scaledValuesOfWaveFrequencies", spectraInfo.scaledValuesFrequencies );
            ...
            std::throw_with_nested(
                Mars2GribConceptException(
                    std::string( waveName ),
                    std::string( waveTypeName<Variant>() ),
                    std::to_string(Stage),
