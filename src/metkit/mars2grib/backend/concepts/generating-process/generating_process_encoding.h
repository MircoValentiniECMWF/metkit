    ...
    if constexpr ( generating_processApplicable<Stage, Section, Variant>() ) {
        ...
        try {
            ...
            LOG_DEBUG_LIB(LibMetkit)
                << "[Concept GeneratingProcess] Op called: "
                << "Stage="   << Stage
                << ", Section=" << Section
                << ", Variant=" << std::string(generating_processTypeName<Variant>())
                << std::endl;
            ...
            std::throw_with_nested(
                Mars2GribConceptException(
                    std::string( generatingProcessName ),
                    std::string( generating_processTypeName<Variant>() ),
                    std::to_string(Stage),
                    std::to_string(Section),
                    "Unable to set `generating_process` concept...",
                    Here()
                )
            );
