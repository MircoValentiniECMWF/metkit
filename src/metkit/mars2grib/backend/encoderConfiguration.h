    ...
    try {
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error stripping descriptor from concept name " + std::string(name),
                Here()
            )
        );
    }
    ...
    try {
        if (!cfg.has("product-definition-section")) {
            throw Mars2GribGenericException(
                "No product definition section in configuration",
                Here()
            );
        }
        ...
        if (!pds.has("template-number")) {
            throw Mars2GribGenericException(
                "No product definition template number in configuration",
                Here()
            );
        }
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error resolving reference time type",
                Here()
            )
        );
    }
...
inline std::string mapStatisticsType(const std::string& stat,
                                     const std::string& name)
{
    ...
    try {
        ...
        if (it == table.end()) {
            throw Mars2GribGenericException(
                "Unsupported type-of-statistical-processing " + stat +
                " for concept " + name,
                Here()
            );
        }
...
inline void
populateConceptsFromYaml(const eckit::LocalConfiguration& cfg,
                         const eckit::LocalConfiguration& sectionCfg,
    ...
    try {
        for (const auto& name : sectionCfg.keys()) {
            ...
            if (!conceptCfg.has("type")) {
                throw Mars2GribGenericException(
                    "No type found for concept " + name,
                    Here()
                );
            }
            ...
            if (key == "model") {
                ...
                const std::string stat =
                    conceptCfg.getString("type-of-statistical-processing");
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error populating concepts from YAML",
                Here()
            )
        );
    }
...
inline SectionCfg
buildSection(const eckit::LocalConfiguration& cfg, uint16_t secId)
{
    ...
    try {
        ...
        if (!sectionCfg.has("template-number")) {
            throw Mars2GribGenericException(
                sectionName + " has no template number",
                Here()
            );
        }

        const uint16_t tmpl =
            static_cast<uint16_t>(sectionCfg.getLong("template-number"));

        const SectionRecipe* recipe = findRecipe(secId, tmpl);

        if (!recipe) {
            throw Mars2GribGenericException(
                "No recipe found for section " + sectionName +
                " template " + std::to_string(tmpl),
                Here()
            );
        }
        ...
        for (const auto& spec : recipe->concepts) {
            ...
            out.concepts.push_back(
                ConceptCfg{ std::string(spec.name), std::move(finalType) }
            );
        }
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error building section configuration",
                Here()
            )
        );
    }
...
inline void
applyCrossPropagationRules(EncoderCfg& cfg)
{
    ...
    try {
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error applying cross-propagation rules",
                Here()
            )
        );
    }
...
inline EncoderCfg
makeEncoderConfiguration(const eckit::LocalConfiguration& cfg)
{
    try {
        EncoderCfg out;
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error making encoder configuration",
                Here()
            )
        );
    }
...
>
inline auto
makeEncoderCallbacks(const EncoderCfg& cfg)
{
    // ---------------------------------------------------------------------------------------------
    ...
    try {
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error making encoder callbacks",
                Here()
            )
        );
    }
...
inline void
printEncoderConfiguration(const EncoderCfg& cfg,
                          std::ostream& os = std::cout)
{
    ...
    try {
        ...
        std::throw_with_nested(
            Mars2GribGenericException(
                "Error printing encoder configuration",
                Here()
            )
        );
    }
