...
void matchDataset_or_throw( const OptDict_t& opt,
    const OutDict_t& out, std::string expectedDataset ){
    ...
    try {
        ...
        if (bool applyChecks = get_opt<bool>( opt, "applyChecks" ).value_or( false ); applyChecks) {
            ...
            if ( actualDataset != expectedDataset ) {
                throw Mars2GribDeductionException(
                    "Dataset does not match the expected value: " +
        ...
        std::throw_with_nested(
            Mars2GribDeductionException(
                "Unable to check `dataset` from the sample",
                Here()
            )
        );
