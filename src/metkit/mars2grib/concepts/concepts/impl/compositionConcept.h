#pragma once

#include <string>
#include <string_view>
#include <vector>
#include <cstdint>
#include <iostream>

#include "../concept_core.h"

// ======================================================
// ENUM OF VARIANTS FOR THIS CONCEPT
// ======================================================
enum class CompositionType : int {
    Chemical = 0,
    Aerosol = 1,
    Optical = 2,
    Chemicaloptical = 3
};

// ======================================================
// COMPILE-TIME TYPELIST
// ======================================================
using CompositionList = ValueList<
    CompositionType::Chemical,
    CompositionType::Aerosol,
    CompositionType::Optical,
    CompositionType::Chemicaloptical
>;

// ======================================================
// VARIANT -> STRING MAPPING
// ======================================================
template<CompositionType T>
constexpr std::string_view compositionTypeName();

#define DEF(T,NAME) \
    template<> constexpr std::string_view compositionTypeName<T>() { return NAME; }

    DEF(CompositionType::Chemical, "Chemical");
    DEF(CompositionType::Aerosol, "Aerosol");
    DEF(CompositionType::Optical, "Optical");
    DEF(CompositionType::Chemicaloptical, "ChemicalOptical");

#undef DEF

// ======================================================
// DEFAULT APPLICABILITY (user will override manually)
// ======================================================
constexpr bool compositionApplicable(int Stage, int Section, CompositionType Variant)
{
    return false;
}

// ======================================================
// MAIN OPERATION
// ======================================================
template<
    int Stage, int Section,
    CompositionType Variant,
    class MarsDict_t,
    class GeoDict_t,
    class ParDict_t,
    class OptDict_t,
    class OutDict_t
>
uint8_t CompositionOp(
    const MarsDict_t&  mars,
    const GeoDict_t&   geo,
    const ParDict_t&   par,
    const OptDict_t&   opt,
    OutDict_t&         out)
{
    std::cout << "[Concept Composition] Op called: "
              << "Stage="   << Stage
              << ", Section=" << Section
              << ", Variant=" << std::string(compositionTypeName<Variant>())
              << std::endl;
    return 0;
}

// ======================================================
// ConceptInfo
// ======================================================
struct CompositionConceptInfo
{
    static constexpr const char* name = "composition";

    template<
        int Stage, int Sec,
        CompositionType Variant,
        class MarsDict_t,
        class GeoDict_t,
        class ParDict_t,
        class OptDict_t,
        class OutDict_t
    >
    static constexpr Fn<
        MarsDict_t,
        GeoDict_t,
        ParDict_t,
        OptDict_t,
        OutDict_t
    > entry()
    {
        if constexpr ( compositionApplicable(Stage, Sec, Variant) ) {
            return &CompositionOp<
                Stage, Sec, Variant,
                MarsDict_t,
                GeoDict_t,
                ParDict_t,
                OptDict_t,
                OutDict_t
            >;
        } else {
            return nullptr;
        }
    }

    template<auto Variant>
    static std::string variantName()
    {
        return std::string(
            compositionTypeName<
                static_cast<CompositionType>(Variant)
            >()
        );
    }
};
